...
void setup() {
  ...
  oscP5 = new OscP5(this,6006);                         // réception
  myRemoteLocation = new NetAddress("127.0.0.1",12000); // envoi à pure data
  datayougo = new NetAddress("192.168.1.2",5005);       // envoi à yougo 
...
void draw() {
  ...
  OscMessage pointloc = new OscMessage("/map");
  pointloc.add(positions[0].x);
  pointloc.add(positions[0].y);
  pointloc.add(30);
  pointloc.add(30);
  oscP5.send(pointloc, datayougo);
  ...
  for (int y = 0; y < rows; y++) {
    for (int x = 0; x < cols; x++) {
      float cc = map(terrain[x][y], -11, 0, 0, 255); 
  ...
  rotateX(PI/3);
  fill(255, 0, 0, 100);
  ellipse(w/2, h, rayon * 5, rayon * 5);
  noFill();
  translate(-w/2, -h/2 - 60);
  for (int y = 0; y < rows-1; y++) {
    beginShape(TRIANGLE_STRIP);
    for (int x = 0; x < cols; x++) {
      vertex(x*scl, y*scl, terrain[x][y] * 40);
      vertex(x*scl, (y+1)*scl, terrain[x][y+1] * 40);
    }
    endShape();
  }
  ...
  fill(255);
  text("X : " + positions[0].x, 20, 40); 
  text("Y : " + positions[0].y, 20, 80); 
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.addrPattern().equals("/map")) {
    String firstValue = theOscMessage.get(0).stringValue(); 
    //String secondValue = theOscMessage.get(1).stringValue();
    //println("chaine " + firstValue);
    String[] liste = split(firstValue, ' ');
    int tabindex = int(liste[0]);
    float profondeur = float(liste[1]);
    //println("index : "+ tabindex + ", " + "valeur : " + profondeur);
    int colonne = floor(tabindex / cols);
    int rangee = tabindex % rows;
    terrain[colonne][rangee] = profondeur;
    return;
  }
}
      ...
      if (inBuffer.substring(0, 1).equals("{")) {
        ...
        if (json == null) {
          ...
          */
          ...
          float p2temp = map(pot2, 0, 1023, 0.01, 5);
